...
class InceptionV3(nn.Module):
    ...
    ckpt_path: str='https://dl.dropboxusercontent.com/s/xt6zvlvt22dcwck/inception_v3_weights_fid.pickle'
    dtype: str='float32'
    ...
    def __call__(self, x, train=True, rng=None):
        ...
        Args:
            ...
            rng (jax.random.PRNGKey | None): Random seed. Defaults to a
                fixed key when not provided.
        ...
        x, spatial_features = InceptionC(channels_7x7=160,
                       params_dict=utils.get(self.params_dict, 'Mixed_6d'),
                       dtype=self.dtype)(x, train, features=self.return_spatial)
...
class Dense(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, unbiased=False):
        x = nn.Dense(features=self.features,
                     use_bias=not unbiased,
